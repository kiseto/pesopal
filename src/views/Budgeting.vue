  ...
  <div class="max-w-7xl mx-auto p-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
      <div class="bg-white rounded-2xl shadow p-8 flex flex-col">
        <div class="text-gray-500 text-lg">Monthly Budget</div>
        <div class="text-3xl font-bold text-gray-800">₱{{ summary.monthlyBudget.toLocaleString() }}</div>
        <div class="text-lg text-gray-500">₱{{ summary.spentThisMonth.toLocaleString() }} spent this month</div>
        <div class="w-full h-3 bg-gray-200 rounded mt-4">
          <div class="h-3 bg-blue-600 rounded" :style="{ width: summary.progress + '%' }"></div>
        </div>
      </div>
      <div class="bg-white rounded-2xl shadow p-8 flex flex-col">
        <div class="text-gray-500 text-lg">Savings Goal</div>
        <div class="text-3xl font-bold text-blue-700">₱{{ summary.savingsGoal.toLocaleString() }}</div>
        <div class="text-lg text-gray-500">₱{{ summary.savedSoFar.toLocaleString() }} saved so far</div>
        <div class="w-full h-3 bg-gray-200 rounded mt-4">
          <div class="h-3 bg-blue-500 rounded" :style="{ width: summary.savingsProgress + '%' }"></div>
        </div>
      </div>
      <div class="bg-white rounded-2xl shadow p-8 flex flex-col">
        <div class="text-gray-500 text-lg">Budget Status</div>
        <div :class="summary.remaining >= 0 ? 'text-green-600' : 'text-red-600'" class="text-2xl font-bold">{{ summary.status }}</div>
        <div class="text-lg text-gray-500">₱{{ summary.remaining.toLocaleString() }} remaining</div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
      ...
      <div class="bg-white rounded-2xl shadow p-8">
        <div class="flex items-center justify-between mb-4">
          <div class="font-semibold text-gray-800">Budget Categories</div>
          <button @click="openCategoryModal" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">+ Add Category</button>
        </div>
        <div v-for="cat in categories" :key="cat.id" class="mb-4">
          ...
          <div class="text-sm text-gray-500">₱{{ cat.spent.toLocaleString() }} / ₱{{ cat.allocated.toLocaleString() }}</div>
      ...
      <div class="bg-white rounded-2xl shadow p-8">
        <div class="flex items-center justify-between mb-4">
          <div class="font-semibold text-gray-800">Savings Goals</div>
          <button @click="openGoalModal" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">+ New Goal</button>
        </div>
        <div v-for="goal in savingsGoals" :key="goal.id" class="mb-4">
          ...
          <div class="text-sm text-gray-500">₱{{ goal.saved.toLocaleString() }} of ₱{{ goal.target.toLocaleString() }}</div>
          <div class="w-full h-2 bg-gray-200 rounded mt-1">
            <div class="h-2 rounded" :style="{ background: goal.color, width: (goal.saved / goal.target * 100) + '%' }"></div>
          </div>
          <div class="text-xs text-gray-500 mt-1">{{ Math.round(goal.saved / goal.target * 100) }}% complete</div>
        </div>
      </div>
    </div>

    <!-- Monthly Budget Progress Chart (placeholder) -->
    <div class="bg-white rounded-2xl shadow p-8 mb-12">
      <div class="font-semibold text-gray-800 mb-4">Monthly Budget Progress</div>
      <div class="w-full h-48 flex items-center justify-center">
        <!-- Replace with chart library if needed -->
        <div class="w-full h-full flex items-end gap-4">
          <div v-for="month in months" :key="month.name" class="flex flex-col items-center w-12">
            <div class="w-8 bg-gray-300" :style="{ height: month.budget / 1000 + 'px' }"></div>
            <div class="w-8 bg-green-500 mt-1" :style="{ height: month.spent / 1000 + 'px' }"></div>
            <div class="text-xs mt-1 text-gray-500">{{ month.name }}</div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="bg-white rounded-2xl shadow-lg w-full max-w-xl p-8 relative">
        ...
        <form @submit.prevent="submitCategory">
          <div class="mb-4">
            ...
            <input type="text" v-model="categoryForm.title" required class="border rounded px-3 py-2 w-full" />
          </div>
          <div class="mb-4">
            <label class="block text-gray-700 mb-2">Allocated Amount</label>
            <input type="number" v-model.number="categoryForm.allocated" required min="0" class="border rounded px-3 py-2 w-full" />
          </div>
          <div class="mb-4">
            <label class="block text-gray-700 mb-2">Time Frame</label>
            <input type="text" v-model="categoryForm.timeFrame" required class="border rounded px-3 py-2 w-full" placeholder="e.g. Monthly" />
          </div>
          <div class="mb-4">
            <label class="block text-gray-700 mb-2">Notes</label>
            <textarea v-model="categoryForm.notes" class="border rounded px-3 py-2 w-full" rows="2"></textarea>
          </div>
          <div class="flex justify-end gap-2 mt-4">
            <button type="button" @click="closeCategoryModal" class="px-4 py-2 rounded bg-gray-200 hover:bg-gray-300">Cancel</button>
    ...
    <div v-if="showGoalModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div class="bg-white rounded-2xl shadow-lg w-full max-w-xl p-8 relative">
        <button @click="closeGoalModal" class="absolute top-2 right-2 text-gray-400 hover:text-gray-600">&times;</button>
        <h2 class="text-xl font-bold mb-4">Add Savings Goal</h2>
        <form @submit.prevent="submitGoal">
          <div class="mb-4">
            <label class="block text-gray-700 mb-2">Title</label>
            <input type="text" v-model="goalForm.title" required class="border rounded px-3 py-2 w-full" />
          </div>
          <div class="mb-4">
            <label class="block text-gray-700 mb-2">Target Amount</label>
            <input type="number" v-model.number="goalForm.target" required min="0" class="border rounded px-3 py-2 w-full" />
          </div>
          <div class="mb-4">
            <label class="block text-gray-700 mb-2">Target Date</label>
            <input type="date" v-model="goalForm.targetDate" required class="border rounded px-3 py-2 w-full" />
          </div>
          <div class="mb-4">
            <label class="block text-gray-700 mb-2">Notes</label>
            <textarea v-model="goalForm.notes" class="border rounded px-3 py-2 w-full" rows="2"></textarea>
          </div>
          <div class="flex justify-end gap-2 mt-4">
            <button type="button" @click="closeGoalModal" class="px-4 py-2 rounded bg-gray-200 hover:bg-gray-300">Cancel</button>
