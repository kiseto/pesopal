...
try {
    ...
    if (!isset($_SESSION['user_id'])) {
        echo json_encode(['success' => false, 'message' => 'User not authenticated', 'debug' => 'No session user_id']);
    ...
    $incomeStmt = $pdo->prepare("SELECT COALESCE(SUM(amount), 0) as total_income FROM transactions WHERE user_id = ? AND type = 'income'");
    $incomeStmt->execute([$user_id]);
    $income = $incomeStmt->fetch(PDO::FETCH_ASSOC);
    $totalIncome = (float)($income['total_income'] ?? 0);
    
    $expenseStmt = $pdo->prepare("SELECT COALESCE(SUM(amount), 0) as total_expenses FROM transactions WHERE user_id = ? AND type = 'expense'");
    ...
    $recentStmt = $pdo->prepare("SELECT * FROM transactions WHERE user_id = ? ORDER BY created_at DESC LIMIT 5");
